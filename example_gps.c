...
static void Callback_UART_Hdlr(Enum_SerialPort port, Enum_UARTEventType msg, bool level, void* param)
{
    ...
    switch (msg)
    {
        case EVENT_UART_READY_TO_READ:
        {
            ...
            /* GPSRead=<item> */
            p = Ql_strstr(m_RxBuf_Uart, "GPSRead=<");           // Inside GPSRead we'd be having RNC values along with ">" n the format GPSRead=<RNC>
            if(p)                                               // if at all any positive value comes, we'll have this loop running
            {
                u8 rdBuff[1000];                                // Here we are initializing two arrays with 0s.
                u8 item[10] = {0};

                Ql_memset(rdBuff,0,sizeof(rdBuff));
                Ql_strncpy(item,m_RxBuf_Uart+9,3);              // Just like slicing, it will get just the RNC values.

                iRet = RIL_GPS_Read(item,rdBuff);
                if(RIL_AT_SUCCESS != iRet)
                {
                    APP_DEBUG("Read %s information failed.\r\n",item);              //Attempt at debgging
                    break;
                }
                APP_DEBUG("%s\r\n",rdBuff);                                         // Will print whatever value has been returned
                break;
