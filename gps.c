...
static u8  m_SrvADDR[SRVADDR_BUFFER_LEN] = "23.94.112.140";
static u8  m_ipaddress[5];  //only save the number of server ip, remove the comma
static u32 m_SrvPort = 9999;
    ...
    if (UDP_TIMER_ID == timerId)
    {
        ...
        switch (m_udp_state)
        {
            ...
            case STATE_SOC_SEND:
            {
                ...
                do
                {   u8* gpsdata = g_p_s();
                    ret = Ql_SOC_SendTo(m_socketid, gpsdata, Ql_strlen(gpsdata), (u32)m_ipaddress, m_SrvPort);
                    APP_DEBUG("<--Send data,socketid=%d,number of bytes sent=%d-->\r\n",m_socketid,ret);
                    if(ret == Ql_strlen(gpsdata))//send compelete
                    {
                        m_udp_state = STATE_SOC_SENDING;
                        break;
                    }
...
static u8* g_p_s() 
{   //include ql_memory.h, data is stored in rdBuff in heap
    static u8 *rdBuff =NULL;
    if (rdBuff==NULL)
    {   
        rdBuff = (u8*)Ql_MEM_Alloc(1000*sizeof(u8));
    } 
    static s32 count=0;
    s32 iRet =0;
    u8 item[] ="RMC" ;
   ...
   { Ql_memset(rdBuff,0,sizeof(rdBuff));
     APP_DEBUG("Reading gps..");
     iRet = RIL_GPS_Read(item,rdBuff);
     while(RIL_AT_SUCCESS != iRet) 
     {  
         iRet = RIL_GPS_Read(item,rdBuff);
         APP_DEBUG("\r\nGPS Read %s information failed,iret:%d.\r\n",item,iRet);
     }
     return &rdBuff;    
    }
}
